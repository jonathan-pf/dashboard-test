...
export function Gen() {
  const genIdeas = useIdeasByType('Gen')
  ...
  const genByYear = useMemo(() => {
    if (!genIdeas) return []

    const grouped = genIdeas.reduce((acc, idea) => {
      ...
      acc[year].push(idea)
      return acc
    }, {} as Record<number, typeof genIdeas>)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Gen</h2>
        <span className="text-sm text-slate-500">
          {genIdeas?.length ?? 0} total
        </span>
      </div>

      {genByYear.map(({ year, items }) => (
                ...
                {editingId === idea.id ? (
                  <div className="mt-2 space-y-2">
                    <textarea
                      ...
                      className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:border-lime-500 outline-none resize-none"
                      rows={3}
                      autoFocus
                    />
                    <div className="flex gap-2 justify-end">
                      ...
                      <button
                        ...
                        disabled={updateIdea.isPending}
                        className="px-3 py-1 text-xs bg-lime-600 text-white rounded-md hover:bg-lime-700 disabled:opacity-50"
                      >

...
export function Features() {
  const features = useIdeasByType('Feature')
  ...
  const featuresByYear = useMemo(() => {
    ...
    const grouped = features.reduce((acc, feature) => {
      const year = getYear(feature.when)
      if (!acc[year]) {
        acc[year] = []
      }
      acc[year].push(feature)
      return acc
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <span className="text-sm text-slate-500">
          {features?.length ?? 0} total
        </span>
      </div>

      {featuresByYear.map(({ year, items }) => (
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    <div className="flex gap-2 justify-end">
                      ...
                      <button
                        ...
                        className="px-3 py-1 text-xs bg-teal-600 text-white rounded-md hover:bg-teal-700 disabled:opacity-50"
                      >

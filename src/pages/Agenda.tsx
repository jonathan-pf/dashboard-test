...
export function Agenda() {
  const agendaIdeas = useIdeasByType('Agenda')
  ...
  const agendaByYear = useMemo(() => {
    if (!agendaIdeas) return []

    const grouped = agendaIdeas.reduce((acc, idea) => {
      ...
      acc[year].push(idea)
      return acc
    }, {} as Record<number, typeof agendaIdeas>)
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-slate-500">
          {agendaIdeas?.length ?? 0} total
        </span>
      </div>

      {agendaByYear.map(({ year, items }) => (
                ...
                {editingId === idea.id ? (
                  <div className="mt-2 space-y-2">
                    <textarea
                      ...
                      className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-violet-500 outline-none resize-none"
                      rows={3}
                      autoFocus
                    />
      ...
      {(!agendaIdeas || agendaIdeas.length === 0) && (

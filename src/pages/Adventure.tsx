...
export function Adventure() {
  const adventureIdeas = useIdeasByType('Adventure')
  ...
  const adventureByYear = useMemo(() => {
    if (!adventureIdeas) return []

    const grouped = adventureIdeas.reduce((acc, idea) => {
      const year = getYear(idea.when)
      if (!acc[year]) {
        acc[year] = []
      }
      acc[year].push(idea)
      return acc
    }, {} as Record<number, typeof adventureIdeas>)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Adventure</h2>
        <span className="text-sm text-slate-500">
          {adventureIdeas?.length ?? 0} total
        </span>
      </div>

      {adventureByYear.map(({ year, items }) => (
                ...
                {editingId === idea.id ? (
                  <div className="mt-2 space-y-2">
                    <textarea
                      ...
                      className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none resize-none"
                      rows={3}
                      autoFocus
                    />
      ...
      {(!adventureIdeas || adventureIdeas.length === 0) && (
        <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200">
          <div className="text-center py-8 text-slate-400">
            No adventure ideas yet
          </div>
        </div>
      )}
    </div>

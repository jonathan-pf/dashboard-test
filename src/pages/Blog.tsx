...
export function Blog() {
  const blogs = useIdeasByType('Blog')
      ...
      {blogsByYear.map(({ year, items }) => (
        <div key={year} className="bg-white rounded-xl p-4 shadow-sm border border-slate-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-slate-500">
              {items.length} blog{items.length !== 1 ? 's' : ''}
            </span>
          </div>
                ...
                {editingId === idea.id ? (
                  <div className="mt-2 space-y-2">
                    <textarea
                      ...
                      className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none resize-none"
                      rows={3}
                      autoFocus
                    />

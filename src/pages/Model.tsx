...
export function Model() {
  const modelIdeas = useIdeasByType('Model')
  ...
  const modelByYear = useMemo(() => {
    if (!modelIdeas) return []

    const grouped = modelIdeas.reduce((acc, idea) => {
      ...
      acc[year].push(idea)
      return acc
    }, {} as Record<number, typeof modelIdeas>)
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-slate-500">
          {modelIdeas?.length ?? 0} total
        </span>
      </div>

      {modelByYear.map(({ year, items }) => (
      ...
      {(!modelIdeas || modelIdeas.length === 0) && (

...
function getCurrentQuarterInfo() {
  ...
  // Default deadline to the last day of the current quarter
...
export function QuarterlyGoals() {
  ...
  const { quarterLabel, defaultDeadline } = getCurrentQuarterInfo()

  const completedGoals = quarterGoals?.filter((g) => g.status === 'Success') ?? []
  const failedGoals = quarterGoals?.filter((g) => g.status === 'Fail') ?? []
  const goalsByArea = groupGoalsByArea(quarterGoals ?? [], areas)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-900">This Quarter's Goals</h2>
        <Link
          to="/goals"
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-slate-900">Goals for This Quarter</h3>
          <span className="text-sm text-slate-500">
            {completedGoals.length} / {quarterGoals?.length ?? 0}
          </span>
        </div>
        ...
        {(!quarterGoals || quarterGoals.length === 0) && !showAddForm && (

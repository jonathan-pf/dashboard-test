...
export function Cruxes() {
  ...
  const cruxesByYear = useMemo(() => {
    if (!cruxes) return []

    const grouped = cruxes.reduce((acc, crux) => {
      const year = getYear(crux.when)
      if (!acc[year]) {
        acc[year] = []
      }
      acc[year].push(crux)
      return acc
    }, {} as Record<number, typeof cruxes>)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Cruxes</h2>
        <span className="text-sm text-slate-500">
          {cruxes?.length ?? 0} total
        </span>
      </div>

      {cruxesByYear.map(({ year, items }) => (
        <div key={year} className="bg-white rounded-xl p-4 shadow-sm border border-slate-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-slate-500">
              {items.length} crux{items.length !== 1 ? 'es' : ''}
            </span>
          </div>
